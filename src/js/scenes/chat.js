...
let listeners = {
      ...
      cheer: console.log,
      crash: console.log,
      clearchat: console.log,
      connected: console.log,
      connecting: console.log,
      connectfail: console.log,
      disconnected: console.log,
      displaystreamer: console.log,
      emoteonly: console.log,
      emotesets: console.log,
      followersonly: console.log,
      history: console.log,
      hosted: console.log,
      hosting: console.log,
      join: console.log,
      ...
      resub: console.log,
      ritual: console.log,
      slowmode: console.log,
      subgift: console.log,
      subscribers: console.log,
      subscription: console.log,
      timeout: console.log,
      unhost: console.log,
      unmod: console.log
}
let chat = {
  ...
  emotes: new Set(),
  sidebar: [],
  space: styles.bodytext.fontSize >> 2,
  time: new Date(),
  unique: 0,
...
chat.createLatest = async function (obj){
  ...
  //parts.background = this.fillRoundedRect(bg, parts.text.getBounds(), styles.bodytext.radius, obj.user.color)
  // parts.background = Phaser.Graphics.fillRoundedRect(parts.text.getBounds(), styles.bodytext.radius, obj.user.color)

  parts.background = this.add.graphics()
    // .fillStyle(+('0x' + RegExp('([0-9A-F]{6})','g').exec(obj.user.color).shift()))
    .fillStyle(obj.user.color)
    .fillRoundedRect(
      parts.text.getBounds().x - this.space,
      parts.text.getBounds().y - this.space,
      parts.text.getBounds().width + this.space * 2,
      parts.text.getBounds().height + this.space * 2,
      styles.bodytext.radius)
  ...
  parts.background.setX(parts.text.getBounds().x).setY(parts.text.getBounds().y).setDepth(0)
  parts.name.setY((parts.text.getBounds().y < parts.flair.getBounds().y && parts.name.getBounds().width > this.AVSIZE ? parts.text.getBounds().y : parts.flair.getBounds().y) - parts.name.getBounds().height - this.space)
...
listeners.message = async function (obj) {
  if (!this.time || this.time.getMinutes() < new Date().getMinutes()) {
    this.time = new Date()
...
//     }
  ...
  chat.getPolyPoints = function ({centerX, centerY, vertices, radius})
  {
    let points = []
    var rot = Math.PI / 2 * 3;
    var x = centerX;
    ...
    {
        points.push(
            centerX + Math.cos(rot) * radius,
            centerY + Math.sin(rot) * radius
        )
        rot += step;
    }
  ...
  // chat.fillRoundedRect = function (key, obj, radius, color) {
  //   let x = +obj.x
  //   let y = +obj.y
...
chat.addListeners = function() {
    Object.keys(listeners).forEach(item => socket.on(item, obj => !!this[item] && this[item](obj)))

    socket.on('message', this.message)
    // socket.on('notice', this.notice)
    // socket.on('ban', console.log)
    // socket.on('bits', this.bits)
    // socket.on('cheer', console.log)
    // socket.on('crash', console.log)
    // socket.on('clearchat', console.log)
    // socket.on('connected', console.log)
    // socket.on('connecting', console.log)
    // socket.on('connectfail', console.log)
    // socket.on('disconnected', console.log)
    // socket.on('displaystreamer', this.displaystreamer)
    // socket.on('emoteonly', console.log)
    // socket.on('emotesets', console.log)
    // socket.on('followersonly', console.log)
    ...
    // socket.on('hosted', console.log)
    // socket.on('hosting', console.log)
    // socket.on('join', console.log)
    // socket.on('logon', console.log)
    // socket.on('message', console.log)
    // socket.on('mod', console.log)
    // socket.on('mods', console.log)
    // socket.on('part', console.log)
    // socket.on('raid', console.log)
    // socket.on('reconnect', console.log)
    // socket.on('resub', console.log)
    // socket.on('ritual', console.log)
    // socket.on('slowmode', console.log)
    // socket.on('subgift', console.log)
    // socket.on('subscribers', console.log)
    // socket.on('subscription', console.log)
    // socket.on('timeout', console.log)
    // socket.on('unhost', console.log)
    // socket.on('unmod', console.log)
...
Scenes.set('Chat', {
  ...
  create: function () {
    ...
    //this.fillRoundedRect('temp1', {x: 100, 100, 400, 400}, 20, 0x362865) // 0xdda7fc
    this.addListeners()
